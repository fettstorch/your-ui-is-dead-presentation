...
<template v-if="$clicks >= 1">
    <div><Typewriter immediate v-if="$clicks >= 1">CONTRACT: Animations may only contain:</Typewriter></div>
...
<div v-if="$clicks >= 7" class="w-40 left-100 text-white absolute bottom-0 h-5 text-3 font-bold text-center anim-in-bottom" style="background: red;">ALL PROBLEMS SOLVED</div>
